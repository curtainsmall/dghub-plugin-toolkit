...
```bash
dgstudio-cli build [插件目录] [--pypi-index URL] [--no-color]
dgstudio-cli --version
```
...
| `auto_suffix` | bool | 构建页「按产物模式加后缀」：开启时包名自动追加 `-self_contained` / `-dependent`（后缀文本可在设置页自定义，存于 `~/.dghub-sdk-studio`） |
...
编译入口声明（Studio 构建时读取，不写入 project.json）：Python 由
...
# GitHub Actions
- name: Build plugin
  run: dgstudio-cli build ./my-plugin
```
